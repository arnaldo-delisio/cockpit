...
1. **On-demand RAG** — pull at query time. **[Read-path status: the retrieval *engine* is built (§7); session-injection is UNBUILT — OPEN-9.]** Knowledge/fact nodes never project (MEM-20), so this is their ONLY route back into a live session. *Direction:* **automatic ambient** recall (not explicit-only), read-only, **decoupled from capture** (don't recreate native memory's write+recall coupling — TOOL-6), precision-biased (silence beats noise — cosine is noisy on short text, MEM-27), visibly marked + killable. *Structure (settle in its build session):* **two-tier trigger** — scope-open seed (always) + a cheap per-turn entity/scope/novelty gate that fires a budgeted top-k pull only when it trips (≈3–5 nodes, ~1–2k tokens, titles+one-liners tiered, agent expands on demand) — i.e. evaluate cheaply every turn, inject rarely; **two integrations** — Claude `UserPromptSubmit→additionalContext` first, then Hermes's provider/system-prompt slot; relevance floor + freshness (§7). Gates the Hermes `memory_enabled` flip (DECISIONS TOOL-6 update). Full direction → DECISIONS OPEN-9.
