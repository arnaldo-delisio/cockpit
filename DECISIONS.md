...
- **OPEN-8 · new-boringscale scope boundary** — new-boringscale (an active venture, distinct from the *archived* boringscale whose native-Hermes memory B2 discarded) is being **captured** into `memory/scopes/boringscale/` by the global hooks but is **not** in the reconciler's `LIVE_SCOPES`/`KNOWN_SCOPES`, so nothing is distilled or projected yet. Open: is it a normal **cockpit-graph scope** (shared main-VM graph) or does it **wall off** in its own VM (MEM-23, if confidential)? Decide before adding it to `LIVE_SCOPES`. Do not wipe the dir. [surfaced 2026-06-24]
