...
1. Run the worker (it routes the source itself):
   ...
   - `--language` passes an ISO-639-1 language hint to Groq (`it`, `en`, `es`, …). Use `auto` to omit it. Ask the user when transcribing meetings/calls; explicit language improves Groq accuracy and latency.
